...
async function getPost(filepath: string): Promise<Post<PostFrontmatter>> {
 ...
 */
async function getInfoByCurrentPath(directoryName: string) {
  // 1. check file exists
  ...
  const isDir = await isDirectory(directoryName)
  // dir 이면 index.mdx, 아니면 현재 위치
  const filePath = isDir ? directoryName + "/index.mdx" : directoryName
  const { frontmatter, serialized } = await getPost(filePath)
